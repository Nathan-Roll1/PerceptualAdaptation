...
author: "Nathan Roll, Meghan Sumner, Emily Goodwin, Robert Hawkins"
...
# Load the data
df_main <- read_csv("~/Documents/df_main.csv", show_col_types = FALSE)
...
summary_stats %>%
  mutate(
    ...
    accuracy_pct = sprintf("%.1f%% (±%.1f%%)", mean_accuracy * 100, se_accuracy * 100)
  ) %>%
  select(condition_label, phase, accuracy_pct, n) %>%
  pivot_wider(names_from = phase, values_from = c(accuracy_pct, n)) %>%
  format_table(col.names = c("Condition", "Training Accuracy", "Testing Accuracy", "N (Training)", "N (Testing)"))
```
...
Our primary finding supports the original study's conclusion that exposure configuration significantly impacts perceptual adaptation. The relative adaptation benefit analysis revealed substantial variation across conditions, with some showing positive adaptation (improved performance from training to testing) and others showing negative adaptation costs. Notably, the "same speaker" condition showed the highest testing phase accuracy, supporting the hypothesis that talker-specific training can be highly effective for perceptual learning.
...
This replication provides converging evidence that perceptual adaptation to L2 speech is significantly influenced by the variability of training exposure, where individual talker variation eclipses variety-level effects.